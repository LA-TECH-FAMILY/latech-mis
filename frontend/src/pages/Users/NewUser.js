  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-3 gap-5 items-start">
        {/* Left — Account Details (2/3 width) */}
        ...
        {/* Right — Assign Roles (1/3 width, sticky) */}
        <div className="sticky top-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h3 className="text-sm font-bold text-gray-700 mb-1">Assign Roles</h3>
            <p className="text-xs text-gray-400 mb-3">Select one or more roles for this user.</p>
            {selectedRoles.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {selectedRoles.map(r => (
                  <span key={r} className="text-[10px] font-semibold px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full capitalize">
                    {r.replace('_', ' ')}
                  </span>
                ))}
              </div>
            ...
            <div className="space-y-1.5">
              {roles.map(r => (
                <label key={r.id} className={`flex items-start gap-2.5 p-2.5 rounded-xl cursor-pointer border transition-colors ${selectedRoles.includes(r.name) ? 'bg-blue-50 border-blue-200' : 'border-gray-100 hover:bg-gray-50'}`}>
                  <input type="checkbox" checked={selectedRoles.includes(r.name)} onChange={() => toggleRole(r.name)} className="rounded mt-0.5" />
                  <div>
                    <p className="text-xs font-semibold text-gray-700 capitalize">{r.name.replace('_', ' ')}</p>
